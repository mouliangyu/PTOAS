...
template <typename OpTy>
static void setCubeBridgeLoadSegmentSizes(OperationState &result,
                                          bool hasStartPosition) {
  auto &segments =
      result.getOrAddProperties<typename OpTy::Properties>().operandSegmentSizes;
  llvm::copy(ArrayRef<int32_t>{1, 1, 1, 1, hasStartPosition ? 1 : 0,
                               hasStartPosition ? 1 : 0},
             segments.begin());
}

static ParseResult parseCubeBridgeLoadStartClause(
    OpAsmParser &parser, SmallVectorImpl<OpAsmParser::UnresolvedOperand> &operands) {
  if (failed(parser.parseOptionalKeyword("start")))
    return success();
  OpAsmParser::UnresolvedOperand startRow;
  OpAsmParser::UnresolvedOperand startCol;
  if (parser.parseLParen() || parser.parseOperand(startRow) ||
      parser.parseComma() || parser.parseOperand(startCol) ||
      parser.parseRParen())
    return failure();
  operands.push_back(startRow);
  operands.push_back(startCol);
  return success();
}

static ParseResult parseCubeBridgeLoadStartTypes(OpAsmParser &parser,
                                                 bool hasStartPosition,
                                                 Type &startRowType,
                                                 Type &startColType) {
  if (!hasStartPosition)
    return success();
  if (parser.parseComma() || parser.parseType(startRowType) ||
      parser.parseComma() || parser.parseType(startColType))
    return failure();
  return success();
}

template <typename OpTy>
static ParseResult parseCubeBridgeLoadOp(OpAsmParser &parser,
                                         OperationState &result) {
  OpAsmParser::UnresolvedOperand source, destination, first, second;
  SmallVector<OpAsmParser::UnresolvedOperand, 2> startOperands;
  if (parseRequiredOperandWithComma(parser, source) ||
      parseRequiredOperandWithComma(parser, destination) ||
      parseRequiredOperandWithComma(parser, first) ||
      parser.parseOperand(second) ||
      parseCubeBridgeLoadStartClause(parser, startOperands) ||
      parser.parseOptionalAttrDict(result.attributes) || parser.parseColon())
    return failure();

  Type sourceType, destinationType, firstType, secondType;
  Type startRowType, startColType;
  bool hasStartPosition = !startOperands.empty();
  if (parser.parseType(sourceType) || parser.parseComma() ||
      parser.parseType(destinationType) || parser.parseComma() ||
      parser.parseType(firstType) || parser.parseComma() ||
      parser.parseType(secondType) ||
      parseCubeBridgeLoadStartTypes(parser, hasStartPosition, startRowType,
                                    startColType))
    return failure();

  setCubeBridgeLoadSegmentSizes<OpTy>(result, hasStartPosition);
  ...
  if (parser.resolveOperand(source, sourceType, result.operands) ||
    ...
    return failure();
  if (hasStartPosition &&
      (parser.resolveOperand(startOperands[0], startRowType, result.operands) ||
       parser.resolveOperand(startOperands[1], startColType, result.operands)))
    return failure();
  return success();
}

template <typename OpTy>
static void printCubeBridgeLoadOp(OpTy op, OpAsmPrinter &printer, Value first,
                                  Value second) {
  printer << " " << op.getSource() << ", " << op.getDestination() << ", "
          << first << ", " << second;
  if (op.getStartRow())
    printer << " start(" << op.getStartRow() << ", " << op.getStartCol() << ")";
  printer.printOptionalAttrDict(op->getAttrs(),
                                /*elidedAttrs=*/{"operandSegmentSizes"});
  printer << " : " << op.getSource().getType() << ", "
          << op.getDestination().getType() << ", " << first.getType() << ", "
          << second.getType();
  if (op.getStartRow())
    printer << ", " << op.getStartRow().getType() << ", "
            << op.getStartCol().getType();
}

ParseResult MteL1L0aOp::parse(OpAsmParser &parser, OperationState &result) {
  return parseCubeBridgeLoadOp<MteL1L0aOp>(parser, result);
}

void MteL1L0aOp::print(OpAsmPrinter &printer) {
  printCubeBridgeLoadOp(*this, printer, getM(), getK());
}

ParseResult MteL1L0bOp::parse(OpAsmParser &parser, OperationState &result) {
  return parseCubeBridgeLoadOp<MteL1L0bOp>(parser, result);
}

void MteL1L0bOp::print(OpAsmPrinter &printer) {
  printCubeBridgeLoadOp(*this, printer, getK(), getN());
}

template <typename OpTy>
static LogicalResult verifyCubeBridgeLoadStart(OpTy op) {
  if (static_cast<bool>(op.getStartRow()) != static_cast<bool>(op.getStartCol()))
    return op.emitOpError("requires start_row and start_col to be specified together");
