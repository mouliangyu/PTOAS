...
from ._surface_types import PartitionTensorView, TensorView, Tile, constexpr as _constexpr_marker
...
def _parse_helper_jit_kernel_signature(py_fn) -> KernelSignature:
    ...
    for param in sig.parameters.values():
        if param.kind not in {
            ...
            if param.kind is inspect.Parameter.KEYWORD_ONLY and param.annotation is _constexpr_marker:
                raise TypeError(
                    f"@pto.jit(entry=False) keyword-only parameter '{param.name}' uses unsupported kernel-module "
                    "compile-time annotation pto.constexpr. Kernel-module ABI does not support "
