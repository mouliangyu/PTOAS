# Minimum Pythonic mapping of test/tilelang_st/npu/a5/src/st/testcase/softmax/softmax.pto
import pto
s = pto.scalar

@pto.to_ir(
    name="online_softmax_update_kernel_2d",  # default to function name if not given
    kernel_kind="vector",
    arch="a5",
...
def softmax_demo(
    ...
    c1_i64    = pto.const(1,     dtype=pto.int64)
    c8_i64    = pto.const(8,     dtype=pto.int64)
    c16_i64   = pto.const(16,    dtype=pto.int64)
    c32_i64   = pto.const(32,    dtype=pto.int64)
    c64_i64   = pto.const(64,    dtype=pto.int64)
    c128_i64  = pto.const(128,   dtype=pto.int64)
    c256_i64  = pto.const(256,   dtype=pto.int64)
    c512_i64  = pto.const(512,   dtype=pto.int64)
    ...
    block_idx     = s.index_cast(idx, block_i64)
    row_base      = s.muli(block_idx, c8)
    _             = s.index_cast(i32, c8)        # block_rows_i32
    row_base_i32  = s.index_cast(i32, row_base)
    remaining_rows= s.subi(arg8, row_base_i32)
    has_rows      = s.cmpi_sgt(remaining_rows, c0_i32)  # optionally overload __gt__
    ...
    seq           = s.index_cast(arg7)
    rows          = s.index_cast(arg8)
    ...
    with pto.if_(has_rows):
        ...
        # 5D type `!pto.tensor_view<?x?x?x?x?xf32>` can be inferred from shape rank
        oldmax_view = pto.make_tensor_view(arg0, shape=sh1, strides=s1)  
        oldsum_view = pto.make_tensor_view(arg1, shape=sh1,  strides=s1)
        qk_view     = pto.make_tensor_view(arg2, shape=h128, strides=s128)
        ...
        # 5D type `!pto.tensor_view<?x?x?x?x?xf32> -> !pto.partition_tensor_view<?x?x?x?x?xf32>` can be inferred from shape rank
        ...
        # ── UB tile allocation ─────────────────────────────
        tile_col = pto.tile_buf_type(
            shape=[8,  1], dtype=pto.float32, valid_shape=[-1,  1], blayout="ColMajor")  # valid=?x1, col_major
        tile_w   = pto.tile_buf_type(
            shape=[8, 128], dtype=pto.float32, valid_shape=[-1, -1])            # valid=?x?
        ...
        with pto.vecscope():
            ...
            ptr_ub = pto.ptr(pto.float32, "UB")          # !pto.ptr<f32, ub>
            ...
            with pto.for_(c0, row_count, step=c1) as row:
                row_qk    = s.muli(row, c128)             # can optionally overload __mul__
                ...
                with pto.for_(c0, c128, step=c64,
                              iter_args=(oldmax_bc, oldsum_bc),
                              results=(vf32, vf32)) as loop:
                    chunk                    = loop.iv          # induction variable %chunk (index) bound by `scf.for %chunk = ...`
                    running_max, running_sum = loop.iter_args

                    chunk_i32      = s.index_cast(pto.int32, chunk)        # arith.index_cast index to i32
                    remaining_cols = s.subi(arg7, chunk_i32)                # arith.subi
                    has_chunk      = s.cmpi("sgt", remaining_cols, c0_i32)  # arith.cmpi sgt

                    # %next_max, %next_sum = scf.if %has_chunk -> (!pto.vreg<64xf32>, !pto.vreg<64xf32>)
                    with pto.if_(has_chunk, results=(vf32, vf32)) as br:
                        with br.then_:
                            chunk_mask, chunk_rest = pto.plt_b32(remaining_cols)
                            ...
                            chunk_exp          = pto.vexpdif(vec, merged_max, chunk_mask, "ODD")
                            ...
                            pto.yield_(merged_max, merged_sum)         # scf.yield
                        with br.else_:
                            pto.yield_(running_max, running_sum)        # scf.yield
                    next_max, next_sum = br.results

                    pto.yield_(next_max, next_sum)                      # scf.yield
