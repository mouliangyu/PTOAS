...
- 向量 UB 访问和向量算子必须位于 `llvm.loop.aivector_scope` dummy loop 内
...
- 采用“前处理 + 主体 + 后处理”三段式模板
- 前处理负责 GM→UB DMA 与必要同步
- 主体在 `aic_vecscope` 内执行被测 VPTO op
- 后处理负责必要同步、UB→GM DMA 与 host compare
...
- 不要求每条 op 自己独立覆盖所有 mode，但不能出现整类场景从未被任何 case 触达
...
1. 先补 matrix，登记新增 case 与覆盖关系
2. 每个 case 单独上板验证
3. 更新 matrix 状态
4. 运行 `test/vpto` 全量板测回归
5. 检查 matrix 与 case 目录、板测结果保持一致
...
- `arith` / `scf` 不是独立被测对象
- 所有向量相关 UB 访问和向量算子都放在 `aic_vecscope` 内
