...
Users write source-level non-kernel function names without `.vector` or `.cube`
ABI suffixes. The object emission path derives public ABI names for VPTO LLVM
modules from the target unit:
...
definition for the same source symbol. The input does not spell
`@vpto_post.vector`; the backend object path derives the required ABI symbol.
