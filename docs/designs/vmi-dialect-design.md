...
`pto.vmi.fma` 不能默认拆成 `mulf + addf`。`bitcast` 只有在当前 layout 下 bit grouping
physically adjacent、且每个对应 physical chunk 的 logical bit footprint 相同时才能 direct；
padding bits 只能流向 result padding bits。否则需要 layout conversion、scratch materialization
