...
3. `slot` 必须是编译期常量。
4. `resume` 必须是所在 block 的第一条操作，`keep` 必须紧邻 `func.return`。
5. 同一 `slot` 的 `keep` 与 `resume` payload 类型必须一致。
6. 一个 `slot` 在被消费前不能再次被覆盖。
7. `keep` 和 `resume` 之间不得插入新的 `pto.store_vfsimt_info`。
8. 第一版要求线性链路，不跨分支/循环。
