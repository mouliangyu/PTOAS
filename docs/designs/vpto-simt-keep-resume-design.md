...
- `pto.keep %x {slot = N} : i32`
  - `call void asm sideeffect "", "{R{4+N}}"(i32 %x)`
- `%y = pto.resume {slot = N} : i32`
  - `%y = call i32 asm sideeffect "", "={R{4+N}}"()`
- `pto.keep %x {slot = N} : i64`
  - `call void asm sideeffect "", "{R{4+N}}"(i64 %x)`
- `%y = pto.resume {slot = N} : i64`
  - `%y = call i64 asm sideeffect "", "={R{4+N}}"()`
...
- asm 字符串为空；固定 `R` 寄存器通过 inline asm constraint 表达。
...
- `simt_stage0` 内的 `pto.keep %tid {slot = 0} : i32`
  - `call void asm sideeffect "", "{R4}"(i32 %tid)`
- `simt_stage1` 内的 `pto.resume {slot = 0} : i32`
  - `%tid2 = call i32 asm sideeffect "", "={R4}"()`
