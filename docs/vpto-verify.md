...
- `vabs` / `vexp` / `vln` / `vsqrt` / `vrelu` / `vnot`：输入和结果必须共享同一 `!pto.vreg` 类型；
...
- `vgatherb` 当前要求 `!pto.mask<b32>`，并按 block gather 语义解释；
- `vselr` / `vselrv2` 要求索引向量使用整数元素，lane 数与主数据向量一致，且索引元素宽度与主数据元素位宽匹配；
