...
for case in CASES:
    ...
    # Compute golden according to tpartmin semantics from template:
    # If src0_valid == dst_valid: use tpart_op with src0 as full operand
    #   - If src1 row_less: min for src1 region, copy src0 for remaining rows
    #   - If src1 col_less: copy src0 full, then min for overlapping region
    ...
    if src0_eq_dst:
        ...
        if src1_eq_dst:
            ...
            # Row_less: min for src1 region, copy src0 for remaining rows
            if src1_vc > 0:
                golden[:src1_vr, :src1_vc] = np.minimum(input1[:src1_vr, :src1_vc], input2[:src1_vr, :src1_vc]).astype(dtype, copy=False)
        ...
        if src0_eq_dst:
            ...
            # Col_less: first copy src1, then min in overlapping region
            ...
            # Row_less: min for src0 region, copy src1 for remaining rows
            if src0_vc > 0:
                golden[:src0_vr, :src0_vc] = np.minimum(input1[:src0_vr, :src0_vc], input2[:src0_vr, :src0_vc]).astype(dtype, copy=False)
