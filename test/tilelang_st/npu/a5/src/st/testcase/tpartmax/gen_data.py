...
for case in CASES:
    ...
    # tpartmax semantics (based on pto-isa TPartBinOps.hpp TCopyPadOp):
    # Algorithm:
    # 1. dst[:] = Min (padding for max operation)
    # 2. dst[0:src0_vr, 0:src0_vc] = src0[0:src0_vr, 0:src0_vc] (copy src0 to dst)
    # 3. dst[0:src1_vr, 0:src1_vc] = max(dst[0:src1_vr, 0:src1_vc], src1[0:src1_vr, 0:src1_vc])
    #    (apply max in src1 valid region)

    src0_eq_dst = (src0_vr == dst_vr and src0_vc == dst_vc)
    src1_eq_dst = (src1_vr == dst_vr and src1_vc == dst_vc)

    if src0_eq_dst and src1_eq_dst:
        # Full max: both src0 and src1 cover entire dst
        golden[:dst_vr, :dst_vc] = np.maximum(input1[:dst_vr, :dst_vc], input2[:dst_vr, :dst_vc]).astype(dtype, copy=False)
    elif src0_eq_dst:
        # src0 covers dst, src1 is partial
        # dst = src0 (copy), then max(dst, src1) in src1 region = max(src0, src1) in src1 region, src0 in rest
        golden[:src1_vr, :src1_vc] = np.maximum(input1[:src1_vr, :src1_vc], input2[:src1_vr, :src1_vc]).astype(dtype, copy=False)
        if src1_vc < dst_vc:
        ...
        # dst = Min, then copy src0 in src0 region, then max(dst, src1) in src1 region
        golden[:src0_vr, :src0_vc] = np.maximum(input1[:src0_vr, :src0_vc], input2[:src0_vr, :src0_vc]).astype(dtype, copy=False)
        if src0_vc < dst_vc:
        ...
        golden[:min_vr, :min_vc] = np.maximum(input1[:min_vr, :min_vc], input2[:min_vr, :min_vc]).astype(dtype, copy=False)
        ...
        if src1_vr > src0_vr and src0_vc > src1_vc:
            # Region [src0_vr:src1_vr, src1_vc:src0_vc] = Min (neither covers)
            # This is correct for tpartmax - padding value is Min
            # For floats, we use -np.inf. For integers, use dtype min.
            if dtype == np.float32:
                ...
                min_val = np.iinfo(np.uint16).min
            elif dtype == np.int32:
            ...
            golden[src0_vr:src1_vr, src1_vc:src0_vc] = min_val
