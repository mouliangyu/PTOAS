...
"""Single source of truth for tpartmax ST test cases.
...
Each case defines:
  ...
  - src1_vshape:     (src1_valid_rows, src1_valid_cols) — src1 valid region.
                     May be smaller than dst valid region for partial max cases.
  - dst_vshape:      (dst_valid_rows, dst_valid_cols) — dst valid region.
  - eps:             tolerance for numpy.allclose (atol and rtol).

tpartmax semantics:
  - If src0_valid == dst_valid: dst[:src1_rows,:src1_cols] = max(src0[:src1_rows,:src1_cols], src1[:src1_rows,:src1_cols])
                                dst[src1_rows:,:] = src0[src1_rows:,:] (copy remaining rows)
                                OR (for col_less) dst[:,:src1_cols] = max(src0[:,:src1_cols], src1[:,:src1_cols])
