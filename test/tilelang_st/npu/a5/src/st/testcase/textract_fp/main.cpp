...
void LaunchTEXTRACT_FP_f32_f16_16x16(uint16_t *a, uint16_t *b, float *fb, float *out, void *stream);

using LaunchFn = void (*)(uint16_t *, uint16_t *, float *, float *, void *);
...
static const TestCase kCases[] = {
    {"fp_f32_f16_16x16", LaunchTEXTRACT_FP_f32_f16_16x16, 16, 16, 16, 16, 16, 16, 16, 16},
};
...
static int RunCase(const TestCase &tc, int deviceId, aclrtStream stream) {
    ...
    std::printf(
        "[INFO] === case: %s (a=%zux%zu, b=%zux%zu, fb=%zux%zu, out=%zux%zu) ===\n",
        tc.name, tc.aRows, tc.aCols, tc.bRows, tc.bCols, tc.fbRows, tc.fbCols, tc.outRows, tc.outCols
    );
