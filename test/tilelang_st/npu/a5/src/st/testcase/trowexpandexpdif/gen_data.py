...
"""Generate input and golden data for trowexpandexpdif ST test cases.
...
# Inline validation for multi-input format (trowexpandexpdif uses src0/src1/dst)
...
for case in CASES:
    ...
    input1 = np.random.randint(1, 5, size=src0_shape).astype(dtype)
    input2 = np.random.randint(1, 5, size=src1_shape).astype(dtype)
    ...
    # exp(src0 - src1_scalar)
    diff = input1[:src0_vr, :src0_vc] - input2[:src1_vr, 0:1]
    golden[:dst_vr, :dst_vc] = np.exp(diff).astype(dtype, copy=False)
