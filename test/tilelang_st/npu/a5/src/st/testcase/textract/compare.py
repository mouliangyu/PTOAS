...
def main():
    ...
    for case in CASES:
        ...
        golden = np.fromfile(os.path.join(case_dir, "golden.bin"), dtype=np.float32).reshape(shape_out)
        output = np.fromfile(os.path.join(case_dir, "output.bin"), dtype=np.float32).reshape(shape_out)
