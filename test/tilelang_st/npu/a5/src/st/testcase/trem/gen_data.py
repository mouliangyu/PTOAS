...
for case in CASES:
    ...
    input1 = np.random.uniform(low=-1000, high=1000, size=src0_tile).astype(dtype)
    input2 = np.random.uniform(low=3, high=100, size=src1_tile).astype(dtype)

    golden = np.zeros(dst_tile, dtype=dtype)
    vr, vc = valid_shape
    golden[:vr, :vc] = np.remainder(input1[:vr, :vc], input2[:vr, :vc]).astype(dtype)
