...
module {
  func.func @two_vecscope_loops() attributes {pto.version_selection_applied} {
    ...
    scf.for %i = %c0 to %c1 step %c1 {
      ...
      %lhs_vec = pto.vlds %lhs[%c0] : !pto.ptr<f32, ub> -> !pto.vec<64xf32>
      %rhs = pto.castptr %c0_i64 : i64 -> !pto.ptr<f32, ub>
      %rhs_vec = pto.vlds %rhs[%c0] : !pto.ptr<f32, ub> -> !pto.vec<64xf32>
      %max = pto.vmax %lhs_vec, %rhs_vec, %mask : !pto.vec<64xf32>, !pto.vec<64xf32>, !pto.mask -> !pto.vec<64xf32>
      %dst = pto.castptr %c8320_i64 : i64 -> !pto.ptr<f32, ub>
      pto.vsts %max, %dst[%c0], %mask : !pto.vec<64xf32>, !pto.ptr<f32, ub>, !pto.mask
    ...
    scf.for %j = %c0 to %c1 step %c1 {
      ...
      %rhs_vec = pto.vlds %rhs[%c0] : !pto.ptr<f32, ub> -> !pto.vec<64xf32>
      %sum = pto.vadd %lhs_vec, %rhs_vec, %mask : !pto.vec<64xf32>, !pto.vec<64xf32>, !pto.mask -> !pto.vec<64xf32>
      %dst = pto.castptr %c64_i64 : i64 -> !pto.ptr<f32, ub>
      pto.vsts %sum, %dst[%c0], %mask : !pto.vec<64xf32>, !pto.ptr<f32, ub>, !pto.mask
    } {llvm.loop.aivector_scope}
    return
  }
}
