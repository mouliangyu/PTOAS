...
def main() -> None:
    golden_scale = np.fromfile("golden_v3.bin", dtype=np.float32)
    scale = np.fromfile("v3.bin", dtype=np.float32)
    golden_out = np.fromfile("golden_v4.bin", dtype=np.uint8)
    out = np.fromfile("v4.bin", dtype=np.uint8)
