...
def generate(output_dir: Path) -> None:
    ...
    f8_row = np.tile(F8E5M2_BYTES, repeats)[:COLS].astype(np.uint8)

    src = np.tile(q_row.astype(np.float16), (ROWS, 1))
    golden_out = np.tile(f8_row, (ROWS, 1)).astype(np.uint8)
    golden_scale1 = np.full(SCALE1_BYTES, np.uint8(0x7F), dtype=np.uint8)
    golden_scale2 = np.zeros(SCALE2_BYTES, dtype=np.uint8)
    golden_scale2[0::2] = np.uint8(0x7F)
