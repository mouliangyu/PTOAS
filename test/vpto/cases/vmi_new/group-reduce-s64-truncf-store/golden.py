...
def generate(output_dir: Path) -> None:
    ...
    dst = np.full(ROWS * OUTPUT_STRIDE, SENTINEL, dtype=np.float16)
    golden = np.full(ROWS * OUTPUT_STRIDE, SENTINEL, dtype=np.float16)
    for row in range(ROWS):
        row_sum = np.sum(src[row, :], dtype=np.float32)
        golden[row * OUTPUT_STRIDE] = np.float16(row_sum)
