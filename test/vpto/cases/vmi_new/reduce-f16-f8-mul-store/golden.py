...
def generate(output_dir: Path) -> None:
    ...
    src_f16 = np.full(ELEMS, F16_VALUE, dtype=np.float16)
    src_f8 = np.tile(F8E4M3FN_BYTES, repeats)[:ELEMS].astype(np.uint8)
    decoded_f8 = np.tile(VALUES, repeats)[:ELEMS].astype(np.float32)
    reduction = np.sum(src_f16.astype(np.float32), dtype=np.float32)
    dst = np.full(ELEMS, SENTINEL, dtype=np.float32)
    golden = decoded_f8 * reduction
