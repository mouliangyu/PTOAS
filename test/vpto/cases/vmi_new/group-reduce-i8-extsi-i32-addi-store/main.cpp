...
void LaunchVmi_group_reduce_i8_extsi_i32_addi_store_kernel(int8_t *src,
                                                           int32_t *dst,
                                                           void *stream);

int main() {
  constexpr size_t kInputElems = 256;
  constexpr size_t kOutputElems = 8;
  size_t srcBytes = kInputElems * sizeof(int8_t);
  ...
  ACL_CHECK(aclrtMemcpy(dstDevice, dstBytes, dstHost, dstBytes,
                        ACL_MEMCPY_HOST_TO_DEVICE));
  LaunchVmi_group_reduce_i8_extsi_i32_addi_store_kernel(srcDevice, dstDevice,
                                                        stream);
