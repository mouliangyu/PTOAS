...
int main() {
  ...
  void *v1Host = nullptr;
  void *v1Device = nullptr;
  void *v2Host = nullptr;
  void *v2Device = nullptr;
  int rc = 0;
  ...
  ACL_CHECK(aclrtMallocHost(&v1Host, fileSize_v1));
  ACL_CHECK(aclrtMallocHost(&v2Host, fileSize_v2));
  ACL_CHECK(aclrtMalloc(&v1Device, fileSize_v1, ACL_MEM_MALLOC_HUGE_FIRST));
  ACL_CHECK(aclrtMalloc(&v2Device, fileSize_v2, ACL_MEM_MALLOC_HUGE_FIRST));
  ...
  ACL_CHECK(aclrtMemcpy(v2Device, fileSize_v2, v2Host, fileSize_v2,
                        ACL_MEMCPY_HOST_TO_DEVICE));
  LaunchVtrc_f16_rounding_kernel_2d(v1Device, v2Device, stream);
