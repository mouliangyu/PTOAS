...
def generate(output_dir: Path, seed: int) -> None:
    ...
    v1.reshape(-1)[LOGICAL_ELEMS:] = 0.0
    v2 = np.zeros((ROWS, COLS), dtype=np.float16)
    golden_v2 = np.zeros((ROWS, COLS), dtype=np.float16)
    golden_v2.reshape(-1)[:LOGICAL_ELEMS] = v1.reshape(-1)[:LOGICAL_ELEMS].astype(np.float16)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate numpy-based inputs/golden for VPTO micro-op vcvt-tail validation."
    )
