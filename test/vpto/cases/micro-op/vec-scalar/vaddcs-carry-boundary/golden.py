...
LHS_PATTERN = np.array(
    [0x00000000, 0x00000001, 0xFFFFFFFE, 0xFFFFFFFF, 0x7FFFFFFF, 0x80000000, 0xAAAAAAAA, 0x55555555],
    dtype=np.uint32,
)
RHS_PATTERN = np.array(
    [0x00000000, 0xFFFFFFFF, 0x00000001, 0x00000000, 0x80000000, 0x7FFFFFFF, 0x55555555, 0xAAAAAAAA],
    dtype=np.uint32,
)
...
def generate(output_dir: Path, seed: int) -> None:
    ...
    total = lhs.astype(np.uint64) + rhs.astype(np.uint64) + np.uint64(1)
    result = (total & np.uint64(0xFFFFFFFF)).astype(np.uint32)
    carry = (total >> np.uint64(32)) != 0
