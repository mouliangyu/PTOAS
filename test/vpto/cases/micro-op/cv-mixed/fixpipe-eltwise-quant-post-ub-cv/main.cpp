...
void LaunchFixpipe_eltwise_quant_post_ub_cv_kernel(__fp16 *src, __fp16 *id,
                                                   __fp16 *elt, __fp16 *out,
                                                   void *stream);

int main() {
  constexpr size_t kSizeSrcElems = 50 * 64;
  constexpr size_t kSizeIdElems = 40 * 50;
  constexpr size_t kEltElems = 64;
  ...
  ACL_CHECK(aclrtMallocHost((void **)&eltHost, kSizeElt));
  ...
  ACL_CHECK(aclrtMalloc((void **)&eltDevice, kSizeElt, ACL_MEM_MALLOC_HUGE_FIRST));
  ...
  FILE_CHECK(ReadFile("./v2.bin", inputSize, srcHost, kSizeSrc) && inputSize == kSizeSrc,
             "./v2.bin");
  inputSize = kSizeElt;
  FILE_CHECK(ReadFile("./v3.bin", inputSize, eltHost, kSizeElt) && inputSize == kSizeElt,
             "./v3.bin");
  inputSize = kSizeOut;
  ...
  ACL_CHECK(aclrtMemcpy(eltDevice, kSizeElt, eltHost, kSizeElt,
                        ACL_MEMCPY_HOST_TO_DEVICE));
  ACL_CHECK(aclrtMemcpy(outDevice, kSizeOut, outHost, kSizeOut,
                        ACL_MEMCPY_HOST_TO_DEVICE));

  LaunchFixpipe_eltwise_quant_post_ub_cv_kernel(srcDevice, idDevice, eltDevice,
                                                outDevice, stream);
