...
def generate(output_dir: Path, seed: int) -> None:
    ...
    golden[0:32] = _pack_prefix(active_lanes=13, bit_stride=1, store_bytes=32)
    golden[32:64] = _pack_prefix(active_lanes=7, bit_stride=2, store_bytes=32)
    golden[64:96] = _pack_prefix(active_lanes=3, bit_stride=4, store_bytes=32)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate packed predicate golden for VPTO micro-op plt-tail-mask validation."
    )
