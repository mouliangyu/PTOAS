...
def generate(output_dir: Path, seed: int) -> None:
    ...
    flat = np.where(v1 > v2, v1, v2).astype(np.float32, copy=False).reshape(-1)
    golden_v3.reshape(-1)[:LOGICAL_ELEMS] = flat[:LOGICAL_ELEMS]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate numpy-based inputs/golden for VPTO micro-op vsel tail validation."
    )
