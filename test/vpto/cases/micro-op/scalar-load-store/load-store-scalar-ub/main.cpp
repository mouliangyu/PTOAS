...
int main() {
    ...
    ACL_CHECK(aclrtMallocHost((void **)(&v2Host), fileSize_v1));
    ACL_CHECK(aclrtMalloc((void **)&v1Device, fileSize_v1, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc((void **)&v2Device, fileSize_v1, ACL_MEM_MALLOC_HUGE_FIRST));

    ReadFile("./v1.bin", fileSize_v1, v1Host, fileSize_v1);
    ReadFile("./v2.bin", fileSize_v1, v2Host, fileSize_v1);
    ACL_CHECK(aclrtMemcpy(v1Device, fileSize_v1, v1Host, fileSize_v1, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(v2Device, fileSize_v1, v2Host, fileSize_v1, ACL_MEMCPY_HOST_TO_DEVICE));
    LaunchLoad_store_scalar_ub_kernel(v1Device, v2Device, stream);

    ACL_CHECK(aclrtSynchronizeStream(stream));
    ACL_CHECK(aclrtMemcpy(v2Host, fileSize_v1, v2Device, fileSize_v1, ACL_MEMCPY_DEVICE_TO_HOST));
    WriteFile("./v2.bin", v2Host, fileSize_v1);
