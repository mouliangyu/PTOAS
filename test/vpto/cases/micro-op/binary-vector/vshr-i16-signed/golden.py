...
def generate(output_dir: Path, seed: int) -> None:
    ...
    v1 = rng.integers(-0x8000, 0x8000, size=ELEMS, dtype=np.int16)
    v2 = rng.integers(0, 16, size=ELEMS, dtype=np.int16)
    v3 = np.zeros(ELEMS, dtype=np.int16)
    golden_v3 = np.right_shift(v1, v2 & np.int16(15)).astype(np.int16, copy=False)
