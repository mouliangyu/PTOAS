...
void LaunchVmi_group_slots_fanout_store_broadcast_kernel(float *src,
                                                         float *sum,
                                                         float *out,
                                                         void *stream);
...
int main() {
  ...
  constexpr size_t kOutElems = 8;
  size_t srcBytes = kSrcElems * sizeof(float);
  size_t sumBytes = kOutElems * sizeof(float);
  size_t outBytes = kOutElems * sizeof(float);
  ...
  LaunchVmi_group_slots_fanout_store_broadcast_kernel(srcDevice, sumDevice,
                                                      outDevice, stream);
