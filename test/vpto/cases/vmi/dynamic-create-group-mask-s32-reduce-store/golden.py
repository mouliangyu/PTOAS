...
def generate(output_dir: Path) -> None:
    active_base = np.linspace(-0.875, 0.625, ACTIVE, dtype=np.float32)
    inactive_base = np.linspace(19.0, 22.5, COLS - ACTIVE, dtype=np.float32)
    src = np.empty((ROWS, COLS), dtype=np.float32)
    for row in range(ROWS):
        src[row, :ACTIVE] = active_base + np.float32(row) * np.float32(0.03125)
        src[row, ACTIVE:] = inactive_base + np.float32(row) * np.float32(1.75)

    copy = np.full((ROWS, COLS), SENTINEL, dtype=np.float32)
    sums = np.full(ROWS, SENTINEL, dtype=np.float32)
    golden_copy = src.copy()
    golden_copy[:, ACTIVE:] = np.float32(0.0)
    golden_sum = np.sum(src[:, :ACTIVE], axis=1, dtype=np.float32).astype(np.float32)
