...
void LaunchVmi_group_reduce_i32_addi_store_kernel(int32_t *src, int32_t *dst,
                                                  void *stream);

int main() {
  constexpr size_t kInputElems = 64;
  constexpr size_t kOutputElems = 8;
  size_t srcBytes = kInputElems * sizeof(int32_t);
  size_t dstBytes = kOutputElems * sizeof(int32_t);
  ...
  ACL_CHECK(aclrtMalloc((void **)&srcDevice, srcBytes,
                        ACL_MEM_MALLOC_HUGE_FIRST));
  ACL_CHECK(aclrtMalloc((void **)&dstDevice, dstBytes,
                        ACL_MEM_MALLOC_HUGE_FIRST));
  ...
  ACL_CHECK(aclrtMemcpy(dstDevice, dstBytes, dstHost, dstBytes,
                        ACL_MEMCPY_HOST_TO_DEVICE));
  LaunchVmi_group_reduce_i32_addi_store_kernel(srcDevice, dstDevice, stream);
