...
void LaunchVmi_group_broadcast_multi_consumer_kernel(float *src, float *sum,
                                                     uint16_t *dense,
                                                     void *stream);
...
int main() {
  ...
  size_t sumBytes = kRows * sizeof(float);
  size_t denseBytes = kElems * sizeof(uint16_t);
  float *srcHost = nullptr;
  float *srcDevice = nullptr;
  float *sumHost = nullptr;
  float *sumDevice = nullptr;
  uint16_t *denseHost = nullptr;
  uint16_t *denseDevice = nullptr;
  int rc = 0;
  ...
  LaunchVmi_group_broadcast_multi_consumer_kernel(srcDevice, sumDevice,
                                                  denseDevice, stream);
