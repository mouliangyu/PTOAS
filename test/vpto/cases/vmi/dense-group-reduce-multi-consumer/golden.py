...
def generate(output_dir: Path) -> None:
    ...
    copy_out = np.full(INPUT_ELEMS, SENTINEL, dtype=np.float32)
    ...
    for row in range(ROWS):
        ...
        golden_sum[row] = np.sum(values, dtype=np.float32)

    output_dir.mkdir(parents=True, exist_ok=True)
    src.tofile(output_dir / "v1.bin")
    sum_out.tofile(output_dir / "v2.bin")
    copy_out.tofile(output_dir / "v3.bin")
    golden_sum.tofile(output_dir / "golden_v2.bin")
    src.tofile(output_dir / "golden_v3.bin")
