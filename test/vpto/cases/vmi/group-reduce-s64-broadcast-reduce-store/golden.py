...
OUTPUT_STRIDE = 8
OUTPUT_ELEMS = ROWS * OUTPUT_STRIDE
SENTINEL = np.float32(-777.0)


def generate(output_dir: Path) -> None:
    src = np.empty(INPUT_ELEMS, dtype=np.float32)
    dst = np.full(OUTPUT_ELEMS, SENTINEL, dtype=np.float32)
    golden = np.full(OUTPUT_ELEMS, SENTINEL, dtype=np.float32)
    ...
    for row in range(ROWS):
        ...
        golden[row * OUTPUT_STRIDE] = np.sum(values * reduction, dtype=np.float32)
