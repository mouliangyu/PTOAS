...
if [ -n "${WHEEL_PLAT_NAME:-}" ]; then
  ...
  python setup.py bdist_wheel --python-tag "${PY_VERSION}" --plat-name "${WHEEL_PLAT_NAME}" --dist-dir "${WHEEL_DIST_DIR}"
else
  python setup.py bdist_wheel --python-tag "${PY_VERSION}" --dist-dir "${WHEEL_DIST_DIR}"
fi
